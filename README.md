...
ブラウザが開き、`http://127.0.0.1:8765/` で一覧・詳細・検索・要約を見られます。

- **全プロジェクトを一画面で見渡せます。** 「整理」「見直し」のように、一覧を眺めながら
  考えたいときに向いています
- **CLI や Claude（MCP）が別プロセスで加えた変更が数秒で反映されます。** 開いたままに
  しておけば、ターミナルでタスクを足したものがそのまま画面に現れます
- **読み取り専用です。** 編集は `task-py` コマンドから行ってください

| オプション | 意味 |
|---|---|
| `--port`, `-p` | 待ち受けるポート（既定 8765） |
| `--no-open` | ブラウザを自動で開かない |

待ち受けは `127.0.0.1` に固定しており、外部からは接続できません。認証を持たないため、
ネットワークへ公開する手段は用意していません。画面に必要な JavaScript はすべて同梱して
あるので、オフラインでも動き、外部へ通信することもありません。
