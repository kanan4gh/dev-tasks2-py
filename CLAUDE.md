...
<!-- SOURCE: （将来: github.com/kanan4gh/claude-guidelines） -->
<!-- UPDATED: 2026-04-02 -->
...
推奨セット以外に必要なドキュメントがあればここに追記する。（このプロダクトでは追加なし）
...
例: PythonでWebアプリを作る場合
- プロダクト固有層: そのプロダクトのドキュメント構成に書き換え
- 技術スタック固有層: Python/uv/FastAPI、リリースコマンド等に書き換え

### 技術スタックを変更するとき（例: TypeScript → Python）
...
1. `claude-guidelines` リポジトリ（将来作成予定）を更新する
2. 各プロジェクトのCLAUDE.mdの汎用層を手動で同期する
3. `UPDATED` の日付を更新して同期済みであることを記録する

### チームで使うとき

- 汎用層は `claude-guidelines` リポジトリを正典として共有する
- プロダクト固有層・技術スタック固有層は各リポジトリで管理する
- 新メンバーは `claude-guidelines` から汎本層をコピーしてプロジェクトに参加する
