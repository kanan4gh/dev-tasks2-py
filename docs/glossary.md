...
### 作業セッション (Work Session)

**定義**: あるタスクに対して実際に作業した1区間。開始時刻・終了時刻・秒数・記録元（タイマー / 手動）を持つ。

**説明**: タスクの `work_sessions` に追記して蓄積し、合計が「作業時間」になる。**タスクの外のファイルではなくタスク自身に持たせる**のは、タスク ID がストレージローカルで `move` のたびに採番し直されるためである（外部に `(project, task_id)` で持つと、移動・改名・削除のすべてに整合処理が必要になる）。

作業セッションの追記では `updated_at` を更新しない。時間の記録はタスク内容の編集ではないからで、これは [完了日時](#完了日時-completed-at) を別フィールドにした理由と同じである。

**記録の経路**:
- タイマー: `task time start --task <id>` → `task time stop`
- 手動: `task time log <id> <duration>`

**関連用語**:
- [タイマー状態](#タイマー状態-timer-state)
- [完了日時](#完了日時-completed-at)

---

### タイマー状態 (Timer State)

**定義**: 実行中タイマーの宣言的な記録。「いつ・どのタスクに対して・何秒のタイマーを開始したか」だけを保持する。

**説明**: `~/.task-py/timer.yaml` に保存する。残り時間・経過時間は**この記録から都度計算**し、カウントダウンした値を保持しない。そのため、書いたプロセスが落ちても・別プロセス（MCP サーバー、ローカル Web GUI）から読んでも・端末がスリープしても、同じ値が得られる。

実行中タイマーは同時に1本のみ。プロセスの生死はタイマーの有効性の判定に使わない（PID は再利用されるため信用できない）。残ってしまった記録への対処は、`task time start` 時に検出してエラーにする（`--force` で置き換え）ことだけで足りる。

**関連用語**:
- [作業セッション](#作業セッション-work-session)

**ファイルパス**: `~/.task-py/timer.yaml`

---

### 完了日時 (Completed At)

**定義**: タスクが `completed` になった時刻。`Task.completed_at` に保持する。

**説明**: `updated_at` は編集のたびに更新されるため完了日の根拠にできない（完了後にタイトルを直すと完了日が失われる）ことへの対処として導入した。

- アーカイブは「片付け」であって「完了の取り消し」ではないため、`completed → archived` では**保持する**
- `open → archived` では設定しない（アーカイブは完了ではない）
- 移行前の完了済みタスクは値を持たないが、`updated_at` からの推測で**埋めない**。誤った日時を永続化すると、後から嘘か本当か区別できなくなるためである。表示は「記録なし」とする
...
**定義**: `~/.task-py/` 配下の監視対象ファイルの mtime とサイズから作る短いダイジェスト。「いま見えているデータが変わったか」を1つの値で表す。

**本プロジェクトでの位置づけ**: ローカル Web GUI が、CLI や MCP サーバーという別プロセスの変更に追随するために使う。値が変わったことを Server-Sent Events でブラウザへ伝え、ブラウザは表示中のものを取り直す。**差分は送らない** — 差分同期は「いま何が表示されているか」という状態をサーバー側にも持つことになり、真実が YAML のファイルであるという原則を壊す。

監視対象には `/api/overview` が返すものをすべて含める（`config.yaml` / 各 `tasks.yaml` / `timer.yaml` / `daily/*.yaml`）。漏れがあると、画面は「変更を監視中」と言いながら古い値を映し続ける。

**実装**: `src/task_web/watcher.py` の `revision()`

---

### ローカル Web GUI

**定義**: `task-py web` で起動し、ブラウザから `~/.task-py/` のデータを見るための面。CLI・MCP サーバーに並ぶ3つ目の入口。

**本プロジェクトでの位置づけ**: GTD の「整理」「見直し」——一覧を眺めながら仕分ける操作——が CLI でも MCP でも埋まっていない穴だったため追加した。CLI は引き続き第一級のインターフェースであり、GUI は同じデータを読む別の面として並立する。

単一利用者がローカルで使う面に限る。IPv4 の `127.0.0.1` にのみ待ち受け、認証を持たず、リモートからのアクセス・HTTPS・マルチユーザーはスコープ外である。v1.4 時点では**読み取りのみ**。
...
**定義**: 同じディレクトリの一時ファイルへ書き出し、`fsync` してから `os.replace()` で本体と差し替える書き込み方式。

**本プロジェクトでの位置づけ**: `~/.task-py/` 配下のすべての YAML がこの方式で書かれる。読み手が観測するのは常に「差し替え前の完全な内容」か「差し替え後の完全な内容」のどちらかであり、**書きかけのファイルは存在しない**。書き込みが途中で失敗しても本体は変更されない。

一時ファイルを同じディレクトリに作るのは、`os.replace()` が不可分なのは同一ファイルシステム内だけで、`/tmp` を経由するとその保証が消えるためである。

**実装**: `src/task_cli/storage/atomic.py` の `write_atomic()`

---

### 排他区間（transaction）

**定義**: `load()` → 変更 → `save()` の全体を1つのプロセスだけが実行できるようにする区間。`<対象ファイル>.lock` への `fcntl.flock` で実現する。

**本プロジェクトでの位置づけ**: CLI と MCP サーバーは別々のプロセスとして同じ `~/.task-py/` を更新する（[ローカル Web GUI](#ローカル-web-gui) は現時点では読み取りだけなので、排他区間に参加しない。書き込み面を作るときに参加する）。`save()` だけを守ってもロストアップデート（あとから保存したほうが、自分が読んだ時点の一覧全体を書き戻して相手の変更を消す）は防げないため、読み取りから書き込みまでを覆う必要がある。区間の境界を決めるのはservice層である。

プロセスが死ねば OS がロックを解放するので、残骸の後始末を書かない。[タイマー状態](#タイマー状態-timer-state)で「プロセスの生死を判定に使わない」と決めたのと同じ理由である。

**実装**: `src/task_cli/storage/atomic.py` の `locked()` と、各ストレージクラスの `transaction()`

---

### 操作対象プロジェクト（ProjectTarget）

**定義**: usecase 層の各操作が「どのプロジェクトを読み書きするか」を表す値。プロジェクト名（`str`）/ Inbox（`None`）/ アクティブ追従（`ACTIVE_PROJECT`）の3つを取る。

**本プロジェクトでの位置づけ**: 呼び出し側が対象を明示できるようにするための語彙。グローバル設定の `activeProject` はプロセスの外にある共有状態であり、これを暗黙の引数にすると、複数のプロセスが同時に動いたときに「画面で選んだタスク」と「実際に書き換わるタスク」が食い違う（タスク ID はストレージローカルなので、別プロジェクトにも同じ ID が存在しうる）。

`None` は Inbox という**実在する保存先**であって「未指定」ではない。未指定は既定値の `ACTIVE_PROJECT` であり、両者を型で区別している。CLI と MCP は既定値を使うため挙動が変わらない。

**実装**: `src/task_cli/usecases/project_target.py`

---

### FileStorage

**定義**: `~/.task-py/projects/<name>/tasks.yaml` または `~/.task-py/inbox/tasks.yaml` への読み書きとバックアップを担うストレージクラス。

**書き込みフロー**: 排他区間を取る → 現在ファイルを `.bak` にコピー → [不可分な置き換え](#不可分な置き換え)で新データを書き込み → 成功したら `.bak` を削除

**実装**: `src/task_cli/storage/file_storage.py`
...
### か行
- [完了日時](#完了日時-completed-at) — ドメイン用語
- [コミット自動タグ付け](#コミット自動タグ付け-commit-auto-tagging) — ドメイン用語

### さ行
- [作業セッション](#作業セッション-work-session) — ドメイン用語
...
- [操作対象プロジェクト（ProjectTarget）](#操作対象プロジェクトprojecttarget) — アーキテクチャ用語

### た行
- [タイマー状態](#タイマー状態-timer-state) — ドメイン用語
- [タスク](#タスク-task) — ドメイン用語
...
### は行
- [排他区間（transaction）](#排他区間transaction) — アーキテクチャ用語
- [不可分な置き換え](#不可分な置き換え) — アーキテクチャ用語
- [ブランチスラッグ](#ブランチスラッグ-branch-slug) — ドメイン用語
- [プロジェクト](#プロジェクト-project) — ドメイン用語

### ら行
- [リビジョン（revision）](#リビジョンrevision) — アーキテクチャ用語
- [レイヤードアーキテクチャ](#レイヤードアーキテクチャ-layered-architecture) — アーキテクチャ用語
- [ローカル Web GUI](#ローカル-web-gui) — アーキテクチャ用語
