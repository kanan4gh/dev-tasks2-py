...
**説明**: `~/.task-py/timer.yaml` に保存する。残り時間・経過時間は**この記録から都度計算**し、カウントダウンした値を保持しない。そのため、書いたプロセスが落ちても・別プロセス（MCP サーバー、将来のローカル Web GUI）から読んでも・端末がスリープしても、同じ値が得られる。
...
**本プロジェクトでの位置づけ**: CLI・MCP サーバー・（将来の）ローカル Web GUI は別々のプロセスとして同じ `~/.task-py/` を更新する。`save()` だけを守ってもロストアップデート（あとから保存したほうが、自分が読んだ時点の一覧全体を書き戻して相手の変更を消す）は防げないため、読み取りから書き込みまでを覆う必要がある。区間の境界を決めるのはservice層である。
